...
public class SimulatorController {
	...
	public HashMap<Integer, String> StockIndex2(Locale locale, Model model,HttpServletResponse response) {
		...
		//요청할 데이터 정보를 셋팅
		stockChart.setInputValue(0, "A005930"); //종목코드 삼성  A005930 ,  엔솔 A373220
		stockChart.setInputValue(1, 49); //ASCII CODE 49 = '1' (기간으로 요청)
		stockChart.setInputValue(2, 20220609); //요청종료일
		stockChart.setInputValue(3, 20220609); //요청***시작****일
		stockChart.setInputValue(5, new int[] {0,1,2,3,4,5,8,18,19}); //요청항목 (API 확인바람)
...
20220530  0  67500  67800  66900  67700  14255484  20220530  100.0  ======== 
	...
	@RequestMapping(value = "/multiStock.po", method = RequestMethod.POST)
	@ResponseBody 
	public HashMap<Integer, Integer> getMultiStock(@RequestBody String filterJSON, Model model,HttpServletResponse response) throws Exception {   
		...
		List<String> multiList = mapper.readValue(filterJSON, new TypeReference<List<String>>(){});
	    int multiSize=multiList.size();
		...
		int chCnt=0;
		for(String singleStr : multiList) {
			System.out.println(singleStr); //선택한종목별 코드가져옴
			
			//요청할 데이터 정보를 셋팅
			stockChart.setInputValue(0, singleStr); //종목코드
			stockChart.setInputValue(1, 49); //ASCII CODE 49 = '1' (기간으로 요청)
			stockChart.setInputValue(2, 20220610); //요청종료일
			stockChart.setInputValue(3, 20220609); //요청***시작****일
			stockChart.setInputValue(5, new int[] {0,1,2,3,4,5,8,18,19}); //요청항목 (API 확인바람)
			stockChart.setInputValue(6, 68); //ASCII CODE 68 = 'D' (일봉 요청)  //일봉 D 월 M77 주 W87 분 m109 
			//stockChart.setInputValue(7,60); //분봉 설정, 60은 60분 
			...
			for(int i=0; i<numData; i++) {
				//int curValue=Integer.parseInt(map.get(Integer.parseInt(stockChart.getDataValue(0,i).toString())));
				//map.get(Integer.parseInt(stockChart.getDataValue(0,i).toString()));
				//Integer.parseInt(stockChart.getDataValue(5,i).toString());
				
			 
				//이 numData가 for가 되면안되고 기존에 map에 넣엇던 최상위 i
				if (singleStrCnt!=0) {
					//System.out.println("값1");
					//System.out.println(map.get(Integer.parseInt(stockChart.getDataValue(0,i).toString())));
					//System.out.println("값2");
					//System.out.println(Integer.parseInt(stockChart.getDataValue(5,i).toString()));
					//System.out.println("값3");
					System.out.println(numData+"갑체크@"+singleStrCnt +"i는 "+i);
					map.put(Integer.parseInt(stockChart.getDataValue(0,i).toString()) ,map.get(Integer.parseInt(stockChart.getDataValue(0,i).toString()))+Integer.parseInt(stockChart.getDataValue(5,i).toString())); //날짜가 key 종가가 value
				}else {
					 //Cnt=0일때
					System.out.println(numData+"갑체크#"+singleStrCnt+"i는 "+i);
					map.put(Integer.parseInt(stockChart.getDataValue(0,i).toString()) ,Integer.parseInt(stockChart.getDataValue(5,i).toString())); 
				}
		...
		for (Entry<Integer, Integer> entrySet : map.entrySet()) {    
			 ...
			 if(checkMax<entrySet.getValue()) {
				 checkMax = entrySet.getValue();
				 maxDay = entrySet.getKey();
			 }
			 
			 if(checkMin>entrySet.getValue()) {
				 checkMin = entrySet.getValue();
				 minDay = entrySet.getKey();
			 }
		...
		System.out.println("멀티 @@max ,day / min, day ==> "+checkMax+"##"+maxDay+"##"+checkMin+"##"+minDay);
